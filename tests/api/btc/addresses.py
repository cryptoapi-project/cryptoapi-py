...
class AddressesTestCase(unittest.TestCase):
    ...
    def test_get_outputs_by_addresses(self):
        response = self.cli.api.btc.addresses.get_outputs_by_addresses([address], status)
        self.assertNotIn("status", response)

    def test_get_utxo_coin_addresses_info(self):
        response = self.cli.api.btc.addresses.get_utxo_coin_addresses_info([address])
        self.assertNotIn("status", response)

    def test_get_utxo_coin_addresses_history(self):
        response = self.cli.api.btc.addresses.get_utxo_coin_addresses_history([address])
        self.assertNotIn("status", response)
