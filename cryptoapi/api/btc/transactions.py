...
class Transactions:
    ...
    def send_transaction(self, _hash):
        ...
        validate_data(
            self._models.btc.requests.send_transaction,
            data
        )

        validators.update({
            200: self._models.btc.responses.send_transaction
        })
    ...
    def decode_transaction(self, _hash):
        ...
        validate_data(
            self._models.btc.requests.decode_transaction,
            data
        )

        validators.update({
            200: self._models.btc.responses.decode_transaction
        })
